...
var idleExpression = null;
var enableIdleExpression = false;
...
(function () {
    ...
    template.onFrameUpdate = function (currentTime, entityManager, eventManager) {
        ...
        if (enableIdleExpression){
          if (idleExpression == null){
            idleExpression = new light.VectorFloat();
            //52个表情，按苹果标准排序。第0个和第7个分别是左右眼睛闭上
            for(var i=0;i<52;i++){
              if(i == 0 || i== 7){
                idleExpression.add(1.0)
              }else{
                idleExpression.add(0.0)
              }
            }
          }
          eventManager.emit(new light.updateAvatarExpressionEvent(idleExpression, entityManager, eventManager));
        }
    }
}());

light.on('UpdateInputEvent',params=>{
    const jsonData = params["event.script.lightsdk.CustomEvent.EnableAvatarIdleExpression"];
    if (jsonData !== undefined && jsonData !== null){
        var enable = jsonData["enable"];
        console.error("<<<<<<<<<<<<script_anomoji.js:EnableAvatarIdleExpression=" + enable + ">>>>>>>>>>>>>>")
        enableIdleExpression = enable;
    }
})
